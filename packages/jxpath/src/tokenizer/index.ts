...
function predicateRegExpAbsorber(str = '', start = 0, end = str.length - 1): TokenPredicateRegExp | ErrorInvalidRegExp {
    let i = start;
...
function* predicateAbsorber(
    str = '',
    start = 0,
    end = max(str.length - 1, 0)
): Generator<PredicateAbsorberYieldType, PredicateAbsorberReturnType, undefined> {
    ...
    const token =
        str[i] === '/' ? predicateRegExpAbsorber(str, i + 1, end) : absorbRightExpressionPredicate(str, i, end);
