import { inferPlatform, isRootToken, type RootToken, type Token } from "./tokenizer.js";
...
type Absorber = typeof uncAbsorber | typeof tdpAbsorber | typeof ddpAbsorber | typeof posixAbsorber ;
...
export type ParsedPath = { type: FileSystem, path: (Token| RootToken)[], firstError?: Token };

function createPathProcessor(path: string) {
    return function (ns: AbsorberKey): undefined | ParsedPath {
...
function resolve(fromStr: string, ...toFragments: string[]): ParsedPath  {
    let firstPathFrom = firstPath(fromStr) ?? firstPath(getCWD())!;
    if (firstPathFrom && firstPathFrom.firstError) {
    ...
    if (!isRootToken(firstPathFrom.path[0])) {
        ...
        firstPathFrom = firstPath(getCWD())!;
    }
...
function resolvePathObjects(from: ParsedPath, ...toFragments: string[]): ParsedPath {
    ...
    if (toFragments.length === 0) { // efficiency+
        return rc;
    }
