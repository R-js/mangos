...
function getCWDDOSRoot() {
    ...
    if (drive[0] >= 'a' && drive[0] <= 'z' && drive[1] === ':') {
        return new PathTokenImpl(TokenValueEnum.ROOT, `${drive.join('')}`, 0, 1);
    }
    return undefined;
}
...
export default function* tdpAbsorber(str = ''): Generator<PathTokenImpl, undefined, undefined> {
    ...
    if (drive[0] >= 'a' && drive[0] <= 'z' && drive[1] === ':') {
        yield new PathTokenImpl(TokenValueEnum.ROOT, `${drive.join('')}`, i, i + 1);
        i = 2;
    }
...
export function* tdpBodyAbsorber(
    ...
    while (i <= end) {
        ...
        if (result) {
            ...
            let token: PathTokenEnumKeys = togglePathFragment[toggle];
            const errors = [];
            if (toggle === 0) {
                switch (value) {
                    case '..':
